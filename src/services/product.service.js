...
class ProductService {
  ...
  async uploadCodes(productId, entries, uploadedBy, batchName) {
    ...
    try {
      ...
      // ── 1. Deduplicate within the uploaded file in memory ──────────────────
      let invalid = 0;
      ...
      for (const entry of entries) {
        const rawCode = String(typeof entry === 'object' ? entry.code : entry).trim();
        const status = (typeof entry === 'object' && entry.status) ? entry.status : 'available';

        if (!rawCode) { invalid++; continue; }

        // Normalise for dedup (lowercase) but keep original for storage
        const normalised = rawCode.toLowerCase();
        if (seen.has(normalised)) { inFileDupCount++; continue; }
        seen.add(normalised);
        unique.push({ code: rawCode, status });
      }
      ...
      // ── 2. Batch INSERT IGNORE using code_hash for deduplication ──────────
      // INSERT IGNORE skips rows that violate UNIQUE KEY uq_sku_code_hash (sku_id, code_hash).
      // The encrypted `code` column uses a random IV so it can't be used for dedup.
      // The `code_hash` column is a deterministic HMAC-SHA256 of the plaintext code.
      const CHUNK = 500;
      let totalAffected = 0;
      const now = new Date();
      ...
      for (let i = 0; i < unique.length; i += CHUNK) {
        ...
        for (const { code, status } of chunk) {
          values.push(
            skuId,
            encrypt(code),    // AES-256-CBC with random IV — for delivery
            hashCode(code),   // deterministic HMAC — for dedup unique constraint
            status,
            'excel_upload',
            batchId,
            uploadedBy,
        ...
        const [result] = await conn.execute(
          `INSERT IGNORE INTO digital_codes
           (sku_id, code, code_hash, status, source, upload_batch, created_by, sold_at, reserved_at)
         VALUES ${placeholders}`,
          values
        );
      ...
      const dbDuplicates = unique.length - inserted;          // codes already in DB
      const duplicates = inFileDupCount + dbDuplicates;    // total dupes reported

      // ── 3. Count newly inserted available codes for inventory update ───────
      const [[{ newAvailable }]] = await conn.execute(
        `SELECT COUNT(*) AS newAvailable
         FROM digital_codes
        WHERE sku_id = ? AND upload_batch = ? AND status = 'available'`,
        [skuId, batchId]
      );

      // ── 4. Increment inventory stock count ─────────────────────────────────
      if (newAvailable > 0) {
        await conn.execute(
      ...
      return {
        totalRows: entries.length,
        validCodes: inserted,
        duplicates,          // in-file dupes + already-in-DB dupes combined
        invalidCodes: invalid,
