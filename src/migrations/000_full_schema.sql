...
CREATE TABLE IF NOT EXISTS api_logs (
    ...
    request_body MEDIUMTEXT NULL COMMENT 'ENCRYPTED',
    response_body MEDIUMTEXT NULL COMMENT 'ENCRYPTED',
    status_code INT NOT NULL,
    response_time INT NOT NULL COMMENT 'milliseconds',
    ip_address VARCHAR(50) NULL,
    supplier_request MEDIUMTEXT NULL COMMENT 'ENCRYPTED — raw request payload sent to supplier',
    supplier_response MEDIUMTEXT NULL COMMENT 'ENCRYPTED — raw response payload received from supplier. Widened from TEXT (64KB) to MEDIUMTEXT (16MB) — a full Gift2Games /products catalog response (1903 items) encrypted+hex-encoded blew past TEXT, confirmed live.',
...
CREATE TABLE IF NOT EXISTS wgcards_topup_orders (
    ...
    target_account VARCHAR(255) NOT NULL COMMENT 'Display value only — first attributeValues entry (e.g. player ID). Full param set is in attribute_values.',
    attribute_values JSON NULL COMMENT 'Full attributeValues[] sent to placeDirectOrder — a dynamic named list (per getDirectParam), not just one field',
    amount DECIMAL(15,2) NOT NULL,
    currency VARCHAR(3) NOT NULL DEFAULT 'USD',
    status ENUM('pending', 'processing', 'confirmed', 'failed', 'cancelled') NOT NULL DEFAULT 'pending'
